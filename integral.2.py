...
mod = SourceModule("""
    ...
    __device__ float integral(float *x, int d){
    ...
    { 
        ...
        in[idx*(d+1)+d]=integral(in+(idx*(d+1)), d);
    }""")
...
d = int(sys.argv[1])# number of dimentions
total = n**d
print ("N :",total)
print ("D :",d)
...
results.to_csv("result_"+str(d)+".tsv", sep="\t", float_format='%.5f')
